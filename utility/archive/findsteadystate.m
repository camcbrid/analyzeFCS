function [outstruct,youtstruct] = findsteadystate(cellstruct,fitparams,ploton)
...
outstruct = struct;
youtstruct = struct;
time = cellstruct.M9.time;
celltyp = fieldnames(cellstruct);

%time vector
t = cellstruct.M9.time;

%loop across cell types
for ii = 1:length(celltyp)
    
    if strcmp(celltyp{ii},'M9')
        %outstruct.(celltypes{ii}) = cellstruct.(celltypes{ii});
        continue
    end
    
    disp(celltyp{ii})
    K = fitparams.(celltyp{ii}).K;
    r = fitparams.(celltyp{ii}).r;
    P0 = fitparams.(celltyp{ii}).P0;
    ...
    [~,minind] = min(abs(time(:)*ones(1,length(tmin)) - repmat(tmin(:)',[length(time),1])));
    [~,maxind] = min(abs(time(:)*ones(1,length(tmax)) - repmat(tmax(:)',[length(time),1])));
    
    dataflds = fieldnames(cellstruct.(celltyp{ii}));
    if ploton
        figure;
        q = 1;
    end
    ...
    for k = 1:length(dataflds)
        ...
        %init
        m1 = zeros(length(maxind),1);
        m2 = zeros(length(maxind),1);
        m3 = zeros(length(maxind),1);
        b1 = zeros(length(maxind),1);
        b2 = zeros(length(maxind),1);
        b3 = zeros(length(maxind),1);
        rsq1 = zeros(length(maxind),1);
        rsq2 = zeros(length(maxind),1);
        rsq3 = zeros(length(maxind),1);
        rmse1 = zeros(length(maxind),1);
        rmse2 = zeros(length(maxind),1);
        rmse3 = zeros(length(maxind),1);
        yout = zeros(length(maxind),1);
        ystdout = zeros(length(maxind),1);
        %loop across cell populations of same type
        for jj = 1:length(maxind)
            %data in desired time window
            y = cellstruct.(celltyp{ii}).(dataflds{k})(minind(jj):maxind(jj),jj);
            y = y(:);
            x = time(minind(jj):maxind(jj));
            x = x(:);
        ...
        %output data
        disp([celltyp{ii},', ',dataflds{k}])
        disp([rsq1,rsq2,rsq3])
        disp([m1,m2,m3])
        disp([b1,b2,b3])
        outstruct.(celltyp{ii}).tmax = tmax;
        outstruct.(celltyp{ii}).tmin = tmin;
        outstruct.(celltyp{ii}).(dataflds{k}).fit = {fitobj1,fitobj2,fitobj3};
        outstruct.(celltyp{ii}).(dataflds{k}).gof = {gof1,gof2,gof3};
        outstruct.(celltyp{ii}).(dataflds{k}).yout = yout;
        outstruct.(celltyp{ii}).(dataflds{k}).m = [m1,m2,m3];
        outstruct.(celltyp{ii}).(dataflds{k}).b = [b1,b2,b3];
        outstruct.(celltyp{ii}).(dataflds{k}).rsq = [rsq1,rsq2,rsq3];
        outstruct.(celltyp{ii}).(dataflds{k}).rmse = [rmse1,rmse2,rmse3];
        outstruct.(celltyp{ii}).(dataflds{k}).ystdout = ystdout;
        
        %output just steady state
        youtstruct.([celltyp{ii},'_',dataflds{k}]) = mean(yout);
        
        %plot data and fits to verify
        if ploton
            subplot(2,3,q);
            plot(t(min(minind):max(maxind)),...
                cellstruct.(celltyp{ii}).(dataflds{k})(min(minind):max(maxind),:));
            hold on;
            for l = 1:length(m1)
